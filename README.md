...
│   │   ├── Alerts.jsx                     # Operational alerts (low stock, pending orders, exceptions)
│   │   ├── Analytics.jsx                  # Revenue trend & business analytics
...
│   │   ├── DashboardOverview.jsx, AnalyticsOverview.jsx, RevenueTrendChart.jsx  # Dashboard/analytics widgets
│   │   ├── bannerManagemen.jsx, NewArrivalsManagement.jsx   # Content management widgets
│   │   ├── ProductForm.jsx                                  # Product create/edit form
│   │   ├── StockAdjustModal.jsx                              # Inventory stock adjustment
│   │   ├── InputField.jsx, SubmitButton.jsx                 # Reusable login form controls
│   ├── api/                      # apiClient.js (shared axios instance), session.js, productJobs.js
│   ├── hooks/                    # useAdminListQuery, useFocusTrap, useDebouncedValue, useIsMobile, useNetworkStatus
│   └── utils/                    # productCategories.js (canonical category/voltage constants), apiError.js
```
...
| `/alerts`            | Operational alerts (low stock, pending orders, payment/shipment exceptions) | Protected (admin) |
| `/analytics`         | Revenue trend & business analytics | Protected (admin) |
...
- **Operational alerts** (`/alerts`) — low stock, pending orders, and payment/shipment exceptions in one feed, reading `GET /api/admin/alerts` directly with no client-invented read/unread state.
- **Analytics** (`/analytics`) — revenue trend and business metrics via `AnalyticsOverview`/`RevenueTrendChart`.
- **Product management** — create/edit products with images via `ProductForm`, using the real Advika Auto category taxonomy (Lights, Horns & Air, Interior & Comfort, Exterior Styling, Electrical & Wiring, Safety & Tools, Spares & Fitting — see `src/utils/productCategories.js`) and every Advika-specific field the backend accepts: MRP, voltage (required for Lights/Electrical & Wiring), specs, vehicle compatibility (12V/24V), purchasable variants, best-seller flag, rating, and review count.
